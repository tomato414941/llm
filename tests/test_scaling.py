...
def test_scaling_row_combines_checkpoint_and_summary() -> None:
    ...
    row = scaling_row(
        checkpoint_path=Path("checkpoints/smoke.pt"),
...
def test_latest_summary_row_matches_checkpoint_name() -> None:
    rows = [
        {"checkpoint_path": "checkpoints/model.pt", "validation_loss": "2.0"},
        {"checkpoint_path": "/tmp/model.pt", "validation_loss": "1.5"},
    ]

    row = latest_summary_row(rows, Path("checkpoints/model.pt"))
