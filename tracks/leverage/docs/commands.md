# Commands

This is the entry point for reusable leverage-track command examples. Keep the
actual command bodies in the document that owns the workflow, and link to them
from here.

## Reusable Command Guides

- LoRA/SFT RunPod training: `tracks/leverage/docs/lora-sft-runpod.md`
- External benchmarks: `tracks/leverage/docs/external-benchmarks.md`
- Reviewed dataset validation and export: `tracks/leverage/datasets/README.md`
- Prompt generation utilities: `tracks/leverage/prompts/README.md`
- RunPod cleanup check: root `README.md`

## What Belongs Here

Use this file as a map, not as a second runbook. Add a link when a command is
intended to be reused. Leave historical one-off commands in
`tracks/leverage/runs/*.md`.
